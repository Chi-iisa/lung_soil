...
consensus <- consensusTMEAnalysis(bulkExp=as.matrix(ex_mel), 
                                  cancerType = "SKCM", 
                                  statMethod = "ssgsea",
                                  immuneScore=T)
...
for (i in seq_along(t_cells)) {
  y_axis <- ((i - 1) %% ncol == 0)
  t_plots[[i]] <- boxplot_axis(samples_mel, consensus, t_cells[i], 
...
ex.sign.st <- scale(t(ex_mel_sign), center=T, scale=T)
...
annotation <- data.frame(Metastatic_site = clin_mel$MET_SITE)
row.names(annotation) <- clin_mel$GEO_ID
annotation_colors <- list(Metastatic_site = c(Liver = "darkorange", Lung = "green3"))
...
library(GSVA)
params <- gsvaParam(as.matrix(ex_mel), TIS.gset)
GSVA <- gsva(params, verbose = FALSE)
GSVA <- as.data.frame(t(GSVA))

# Save data
write.table(GSVA, file="figures_def2026/results/melanoma_tis_signature.txt", sep="\t", col.names = T, row.names = T, quote = F)
...
# expression data
gene_expression <- as.data.frame(ex_mel)
