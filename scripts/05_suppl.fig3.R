...
consensus <- consensusTMEAnalysis(bulkExp=as.matrix(ex_pancan), 
                                  cancerType = "PAAD", #Pancreatic adenocarcinoma 
                                  statMethod = "ssgsea",
                                  immuneScore=T)
...
for (i in seq_along(t_cells)) {
  y_axis <- ((i - 1) %% ncol == 0)
  t_plots[[i]] <- boxplot_axis(samples_pancan, consensus, t_cells[i], 
...
## Suppl Figure 3C
# Prepare data
ex_pancan_sign <- ex_pancan[sig_genes_pancan,]

ex.sign.st <- scale(t(ex_pancan_sign), center=T, scale=T)
...
annotation <- data.frame(Metastatic_site = clin_pancan$TUMOR_SITE)
row.names(annotation) <- clin_pancan$SAMPLE_ID
...
library(GSVA)
params <- gsvaParam(as.matrix(ex_pancan), TIS.gset)
...
# expression data
gene_expression <- as.data.frame(ex_pancan)
